...
| Scan tool shows "Communication Error" / never finishes AUTO-LINK, and `candump can0` shows a flood of `can0  000  [0]` lines while the scan tool is plugged in but stays silent when the scan tool is unplugged | **Missing 120 Ω bus terminator at the OBD-II connector.** The `000 [0]` lines are CAN error frames caused by signal reflections on an under-terminated bus. Most consumer scan tools (Innova 5210, Autel AL319, ELM327 clones) do not carry their own terminator. Fix: wire a 120 Ω resistor (1/4 W, brown-red-brown) between OBD-II pin 6 and pin 14 right at the connector body. Verify with multimeter: ~60 Ω across pins 6 ↔ 14 (Pi off, scan tool unplugged) means both terminators are in place. See `docs/wiring.md` Connection 3 and `docs/wiring_walkthrough.md` Connection 3.5. |
| `candump can0` shows non-standard IDs at lower bitrate (e.g. `70F`/`71F` at 250 kbps), or no traffic at 500 kbps | Bitrate mismatch or under-termination. First confirm the 120 Ω terminator above is installed (it cures most cases). If errors persist at 500 kbps with proper termination, double-check the MCP2515 crystal frequency matches the `oscillator=` value in `/boot/firmware/config.txt` — common modules ship with 8 MHz or 16 MHz crystals, not interchangeable. |
| `sudo ip -details link show can0` reports `BUS-OFF` or `ERROR-PASSIVE` | The chip gave up after too many transmit errors. Bring the interface down and back up: `sudo ip link set can0 down && sudo ip link set can0 up type can bitrate 500000 restart-ms 100`. Then investigate the cause (termination, crystal, wiring) — recovery alone doesn't fix the underlying problem. |
